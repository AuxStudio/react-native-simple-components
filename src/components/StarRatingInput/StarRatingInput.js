    ...
    return (
      <View style={[styles.container, this.props.style]}>
        {starsArray.map((item, index) => {
          const text = this.props.labelTextValues &&
            this.props.rating === index + 1 && (
              <AnimateOpacity
                ...
                finalValue={1}
                shouldAnimateIn
                style={[styles.textContainer, this.props.labelTextContainerStyle]}
              >
                <Text style={[styles.text, this.props.labelTextStyle]}>
          ...
          return (
            <View key={'star-' + index} style={styles.starContainer}>
              <Touchable
                onPress={() => this.handlePress(index + 1)}
                style={[styles.star, this.props.iconContainerStyle]}
              >
                <AnimateScale
                  initialValue={1}
                  finalValue={1.25}
                  shouldAnimateIn={index === this.state.animateIndex}
                  animateInCallBack={this.setAnimateIndex}
                  shouldAnimateOut={index !== this.state.animateIndex}
                >
                  {item}
                </AnimateScale>
              </Touchable>
