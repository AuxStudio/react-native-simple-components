...
export default class Menu extends React.Component {
  ...
  renderItem({ item }) {
    return (
      <Touchable style={styles.menuItemContainer} onPress={() => this.props.handleSelect(item)}>
        <Text style={[styles.menuItemText, this.props.textStyle]}>{item}</Text>
      </Touchable>
    );
  }
  ...
  render() {
    ...
    const separatorsHeight = this.props.showSeparator ? this.props.values.length - 1 : 0;

    const finalHeight =
      this.props.values.length * (this.props.itemHeight ? this.props.itemHeight : 40) +
      separatorsHeight;

    return (
      <View style={[styles.menuItemsWrapper, minWidthStyles, shadowStyles, this.props.style]}>
        <AnimateHeight initialValue={0} finalValue={finalHeight} shouldAnimateIn>
          <FlatList
            ...
            ItemSeparatorComponent={
              this.props.showSeparatorLine
                ? () => <View style={[styles.separator, separatorColorStyles]} />
                : null
            }
          />
        </AnimateHeight>
